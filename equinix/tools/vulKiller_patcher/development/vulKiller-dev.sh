...
func_reboot_server() {
    ...
    if [ ! -d "$path_to_reboot_schedule" ]; then
        ...
        reboot_time_schedule_count=$(find $path_to_reboot_schedule -maxdepth 1 -type f | wc -l)

        case $reboot_time_schedule_count in

            # If there is no file, the server will be rebooted immediately:
            0)
                func_log "$INFO" "No reboot schedule found. The server will be rebooted immediately." "both"
                reboot_scheduled=0
            ;;

            # If there is one file, creates new variables to parameterize the reboot schedule:
            1)
                func_log "$INFO" "One reboot schedule found. Validating..." "both"

                # Specify the file format using regex (0100 represents 1:00 AM, 2359 represents 11:59 PM, etc.):
                ...
                if [ "$reboot_aborted" -eq 1 ]; then
                    ...
                    case $reboot_scheduled in
                        0)
                            ...
                            shutdown -rf +1 "The server will reboot to apply the new kernel/security fixes. Please save your work and log out." 2> /dev/null
                        ;;

                        1)
                            func_log "$INFO" "Reboot schedule found. The server will be rebooted at the scheduled time: ${reboot_time_schedule_hour}:${reboot_time_schedule_min}." "both"
...
echo "$bar Updating Repository Cache $bar"

func_log "$INFO" "Obtaining instructions from the repositories." "both"
...
if wc -l "$file_updates_available" | grep -w ^"1" > /dev/null ; then
    ...
    func_log "$INFO" "No updates are available for this system." "both"
